    ...
    function portalFindSpreadsheetSessionMatchingOverride(ov, viewDay){
      ...
      for(let i = 0; i < list.length; i++){
        const s = list[i];
        if(!s) continue;
        if(portalNormKeyStr(s.staffId) !== aid) continue;
      ...
      if(portalScheduleOverrideAnchorIsOpenSlot(ov && ov.anchor_client_id)){
        for(let k = 0; k < list.length; k++){
          const s2 = list[k];
          if(!s2) continue;
          if(portalNormKeyStr(s2.staffId) !== aid) continue;
      ...
      for(let j = 0; j < srcRows.length; j++){
        ...
        if(!portalStaffMatchesStatusInstructor(aid, row.instructors)) continue;
      ...
      if(portalScheduleOverrideAnchorIsOpenSlot(ov && ov.anchor_client_id)){
        for(let j2 = 0; j2 < srcRows.length; j2++){
          ...
          if(!portalStaffMatchesStatusInstructor(aid, row2.instructors)) continue;
