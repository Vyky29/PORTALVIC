...
(function (global) {
  ...
  var ACCORDION_BY_TITLE = {
    ...
    "my records": {
      ...
      chips: ["Reports", "Certificates", "Documents", "Payslips", "Uniform"],
    },
    training: {
      theme: "training",
      icon:
  ...
  function portalPrepareQuickMenuAccordionGroup(grp) {
    ...
    if (id && SKIP_GROUP_IDS[id]) {
      ...
      grp.classList.add("menu-group--accordion-ready");
      return;
    }
  ...
  function portalHideSkippedQuickMenuGroups(main) {
    try {
      ...
      for (var id in SKIP_GROUP_IDS) {
        ...
        if (!grp) continue;
        if (id === "portalQuickMenuNotificationsGroup") continue;
        grp.hidden = true;
