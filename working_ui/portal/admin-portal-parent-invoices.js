...
(function (global) {
  ...
  function createInvoiceEmbedHtml() {
    return (
      ...
      '<p class="muted" style="margin:8px 0 10px;max-width:48rem;overflow-wrap:break-word">For <strong>new</strong> places confirmed from Booking Portal after the term has started (or starting mid-term). Builds <strong>1 invoice</strong> for remaining sessions. <strong>GoCardless</strong>: if they finish after this month\'s collection day (1st), first instalment = <strong>bank transfer now</strong> (current-month remainder); later months on the <strong>1st via GoCardless</strong>. On/before the 1st: first instalment on booking day via GoCardless, then remaining 1sts. <strong>Flexi</strong>: first half on the fixed term due (e.g. Autumn 15 August), or <strong>booking day</strong> if that date has passed; second on the mid-term date.</p>' +
