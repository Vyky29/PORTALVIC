/**
 * LOCAL Sessions Overview — day board = Places/Services phases + Covers snaps.
 * Open: http://127.0.0.1:8765/sessions_overview_local.html
 *
 * Standing AS / weekend / physical / climbing / multi: `_local_booking_occupants.json`
 * (Places phases). Bespoke who-works: **Timetable** hours (point 3) + client from
 * Places/Services seat lines. Day Centre: Timetable hours + day board / Covers snaps.
 * Do not invent AS/Bespoke books from canonical for this local chain page.
 *
 * Read-only. Live writes stay on Admin → Sessions Overview / Schedule & Covers.
 */
(function (global) {
  "use strict";
   ...
   */
  function resolveLocalRosterSource(occupantsBySlotId) {
    var C = global.PortalRosterCanonical;
    ...
    return {
      ...
      rosterSourceNote:
        "LOCAL capacity chain (Places + Services DC phases + Timetable + Covers)",
    };
  }

  function boot() {
    var root = document.getElementById("solHubRoot");
    if (!root) {
      setWarn("Missing #solHubRoot.");
      return;
    }
    if (!global.AdminSessionsHub || typeof global.AdminSessionsHub.mount !== "function") {
      setWarn("AdminSessionsHub failed to load.");
      return;
    }

    setWarn("");
    root.innerHTML =
      '<p class="muted" role="status" style="margin:12px 0;font-size:14px">Loading local Overview…</p>';

    Promise.all([
      fetchJson("/portal/_local_schedule_overrides.json")
        .then(function (data) {
          return flattenOverrides((data && data.byDate) || {});
        })
        .catch(function () {
          return [];
        }),
      fetchJson("/portal/_local_staff_unavailability.json")
        .then(function (data) {
          return flattenUnavailability((data && data.byDate) || {});
        })
        .catch(function () {
          return [];
        }),
      ...
      var overrides = pack[0] || [];
      var unavail = pack[1] || [];
      var occupants = pack[2] || {};
      var phaseCount = Object.keys(occupants).filter(function (id) {
        return isPlacesPhaseServiceId((occupants[id] || {}).serviceId);
      }).length;

      var Chain = global.PortalOverviewCapacityChain;
      global.STAFF_DASHBOARD_SOURCE =
        Chain && typeof Chain.resolve === "function"
          ? Chain.resolve({ bySlotId: occupants }) || resolveLocalRosterSource(occupants)
          : resolveLocalRosterSource(occupants);
      /* Keep local merge if hub refresh re-resolves roster (otherwise Fri Tinashe
         comes back as 4.15-6.15 staff band and duplicates the kid card). */
      global.portalResolveStaffDashboardSource = function (opts) {
        if (opts && opts.forSessionsOverview === false) {
          return global.STAFF_DASHBOARD_SOURCE;
        }
        return global.STAFF_DASHBOARD_SOURCE;
      };
      markRosterReady();
      try {
        global.__PORTAL_SCHEDULE_OVERRIDE_ROWS__ = overrides.slice();
        global.__PORTAL_STAFF_UNAVAILABILITY__ = unavail.slice();
      } catch (_g) {}

      var payload = {
        schedule_overrides: overrides,
        staff_unavailability: unavail,
        session_feedback: [],
        session_quick_marks: [],
        incident_reports: [],
      ...
      var missing = [];
      if (!overrides.length) missing.push("overrides snap empty or missing");
      if (!unavail.length) missing.push("unavailability snap empty or missing");
      if (!phaseCount) missing.push("occupants phases empty — AS board will miss Places seats");
      if (!(global.STAFF_DASHBOARD_SOURCE.rows || []).length) {
        missing.push("roster rows empty");
      }
      if (missing.length) {
        setWarn(
          "LOCAL Overview loaded with gaps: " +
            missing.join("; ") +
            ". AS paints from Places phases when occupants snap is present.",
        );
      }

      return global.AdminSessionsHub.mount(root, {
        mode: "tracking",
        tab: "tracking",
        externalTabs: true,
        payload: payload,
      ...
      if (!hub) return;
      try {
        /* LOCAL: never wait on Supabase roster-ready — paint the day body directly. */
        hub.overviewRosterReadyToPaint = function () {
          return true;
        };
        hub.scheduleOverviewBodyPaint = function () {
          var mount = root.querySelector("[data-ash-overview-body]");
          if (!mount) return;
          try {
            mount.innerHTML = hub.htmlTrackingBody();
          } catch (err) {
            console.warn("[SessionsOverviewLocal] day board", err);
            mount.innerHTML =
        ...
        hub._forceFullOverviewRender = true;
        hub._overviewWaitStartedAt = 0;
        global.__PORTAL_STAFF_ROSTER_LIVE_READY__ = true;
        hub.render();
        hub.scheduleOverviewBodyPaint();
      } catch (err) {
        setWarn("Overview paint failed: " + (err && err.message ? err.message : String(err)));
      }
    }).catch(function (err) {
      setWarn("Overview boot failed: " + (err && err.message ? err.message : String(err)));
    });
  }

  global.SessionsOverviewLocal = { boot: boot };
