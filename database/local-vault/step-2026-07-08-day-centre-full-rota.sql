...
-- Base bundle for this future week is wrong (Youssef on Emmanuel/Fadi, Roberto on
-- Fadi, HOME Victor, Emmanuel only 11-1). Correct staffing:
--   Ikram   11-3 Luliya + Youssef ; 3-4 Victor            (set in prior step)
--   Emmanuel 11-3 Victor          ; 3-4 Raul
--   Fadi    12.30-3 Raul
...
where session_date = '2026-07-08'::date
  and lower(trim(client_name)) in ('emmanuel', 'fadi', 'home')
  and status = 'active'
...
cross join (
  values
    ('Emmanuel', '11 to 1', 'YOUSSEF', 'Day Centre', 'Hub Room'),
...
-- Correct Wed 8 Day Centre rota (Emmanuel + Fadi; Ikram handled in prior step).
insert into public.portal_roster_rows (
  client_name, day, time_slot, instructors, service, area, venue, session_date, status, created_by, updated_by
)
...
cross join (
  values
    ('Emmanuel', '11 to 3', 'VICTOR', 'Day Centre', 'Hub Room'),
    ('Emmanuel', '3 to 4', 'RAUL', 'Day Centre', 'Hub Room'),
...
where session_date = '2026-07-08'::date
  and lower(trim(client_name)) in ('ikram', 'emmanuel', 'fadi', 'home')
