#!/usr/bin/env node
/**
 * Build step-announcement-push.local.sql from template + PORTAL_PUSH_WEBHOOK_SECRET.
 */
import fs from "fs";
...
const templatePath = path.join(__dirname, "step-announcement-push.template.sql");
const outPath = path.join(__dirname, "step-announcement-push.local.sql");
