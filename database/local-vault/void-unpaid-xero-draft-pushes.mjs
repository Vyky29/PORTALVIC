#!/usr/bin/env node
/**
 * Void/delete Xero ACCREC invoices that were pushed too early (unpaid /
 * pending / void Portal drafts from re-enrol or booking). Clears Portal
 * xero_* links so Confirm paid can recreate later.
 *
 *   node database/local-vault/void-unpaid-xero-draft-pushes.mjs
 *   node database/local-vault/void-unpaid-xero-draft-pushes.mjs --dry-run
 */
...
async function xeroAccessToken() {
  ...
  if (!res.ok || !json.access_token) {
    throw new Error(`xero_auth ${res.status}: ${json.error || ""} ${json.error_description || ""}`);
  }
...
async function getInvoice(token, id) {
  ...
  const json = await res.json().catch(() => ({}));
  if (!res.ok) {
    return { ok: false, status: res.status, detail: json?.Message || json?.Detail || JSON.stringify(json).slice(0, 200) };
  }
...
async function clearPortalLink(shareId) {
  return sb(`portal_parent_invoice_share?id=eq.${shareId}`, {
    ...
    body: JSON.stringify({
      ...
      xero_push_error: "cleared_early_xero_push",
...
const rows = await sb(
  "portal_parent_invoice_share?xero_invoice_id=not.is.null&select=id,invoice_number,payment_status,created_via,amount_gbp,xero_invoice_id,xero_payment_id&order=invoice_number",
);
const targets = (rows || []).filter((r) =>
  ["unpaid", "void", "partial", "pending_confirmation"].includes(
    String(r.payment_status || "").toLowerCase(),
  )
);

console.log(DRY ? "DRY RUN" : "LIVE", "targets", targets.length);
...
for (const row of targets) {
  ...
  const line = { invoice_number: invNo, portal_status: row.payment_status, xero_invoice_id: xeroId };
  try {
    ...
    if (!inv.ok) {
      // Already gone in Xero — still clear Portal link.
      line.xero = "missing";
    ...
    if (["VOIDED", "DELETED"].includes(inv.status)) {
      ...
      // Remove payments so AUTHORISED/PAID can be voided.
      const paymentIds = [
        ...new Set(
          [
      ...
      if (!DRY && paymentIds.length && ["PAID", "AUTHORISED"].includes(inv.status)) {
        for (const pid of paymentIds) {
          ...
          if (!del.ok) {
            line.payment_delete_error = del.detail;
          }
