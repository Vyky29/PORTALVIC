-- Admin/CEO (and portal ops admin): read staff policy acknowledgments for the matrix.
-- Also expose a SECURITY DEFINER RPC so the matrix works even if RLS policies differ.
...
comment on function public.portal_admin_list_staff_policy_acks() is
  'Admin matrix: list staff_policy_ack rows (related_session_key = POL-id).';
