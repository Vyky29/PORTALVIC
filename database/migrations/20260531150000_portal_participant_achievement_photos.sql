...
comment on function public.portal_staff_is_staff_or_lead() is
  'True when auth user is staff_profiles app_role staff or lead.';
...
comment on table public.portal_participant_achievement_photos is
  'Session-day achievement photos per participant; staff see drafts only until feedback submit.';
...
create index if not exists portal_achievement_photos_feedback_idx
  on public.portal_participant_achievement_photos (session_feedback_id)
  where session_feedback_id is not null;

create index if not exists portal_achievement_photos_admin_day_idx
  on public.portal_participant_achievement_photos (session_date desc, created_at desc);
...
-- Private bucket: path {staff_user_id}/{session_date}/{client_id}/{photo_id}.jpg
...
create policy portal_achievement_storage_select_admin
  on storage.objects
  for select
  to authenticated
  using (
...
begin
  ...
  if not public.portal_staff_is_staff_or_lead() then
  ...
  return jsonb_build_object(
    'attached', v_attached,
    'archived_unused', v_archived
  );
end;
$$;

revoke all on function public.portal_finalize_achievement_photos(uuid, uuid[], text, date) from public;
