-- Autumn 26/27 afternoon swimming instructor rates + Giuseppe dual rate.
-- Afternoon SI: Simon £24, Roberto £24, Luliya £22, Giuseppe £24, Aurora/Dan/Javier £28.
-- Morning Day Centre / support: leave existing SW rows (halftime morning rates TBD).
-- Giuseppe: morning (incl. pool) stays Support Worker £20; afternoon swim uses SI £24
...
-- ========== GIUSEPPE (Josep): SW £20 primary + SI £24 for afternoon swim ==========
update public.staff_role_rates srr
...
from public.staff_profiles sp
cross join (values
  ('Support Worker',      'Scale 2', 20.00::numeric, true),
  ('Swimming Instructor', 'Scale 2', 24.00::numeric, false)
) as v(role, scale, hourly_rate, is_primary)
where lower(coalesce(sp.username, '')) = 'giuseppe'
