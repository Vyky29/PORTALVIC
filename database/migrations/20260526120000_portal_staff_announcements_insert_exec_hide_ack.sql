-- Announcements composer: allow admin-dashboard operators (manager/admin staff_role) to insert,
-- including rows with hide_after_ack_* after worker signs.
-- Fixes: "new row violates row-level security policy for table portal_staff_announcements"
-- when publishing with auto-hide duration from admin_dashboard.
