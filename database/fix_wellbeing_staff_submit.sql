-- Wellbeing check-in: allow staff resubmit after admin opened 1-to-1 (status in_progress).
-- Run once in Supabase SQL Editor: project cklpnwhlqsulpmkipmqb
-- Fixes: "row-level security policy" when re-testing check-in as staff.

-- 1) Broaden staff update policy
...
-- 2) Trusted upsert for staff (bypasses RLS edge cases on resubmit)
