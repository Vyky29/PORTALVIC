...
Deno.serve(async (req) => {
  ...
  const { error } = await admin.from("onboarding_applicant_drafts").upsert({
  ...
  if (error) {
    console.error("[onboarding-draft-save]", error);
    return json(500, { ok: false, error: "save_failed" });
  }

  if (staffName) {
    await admin.from("onboarding_applicant_sessions").upsert({
      applicant_session_id: sid,
