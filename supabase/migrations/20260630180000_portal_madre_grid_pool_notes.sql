...
update public.portal_roster_rows set area = 'Hub Room', updated_at = now() where status = 'active' and session_date = '2026-06-15'::date and lower(trim(client_name)) = lower(trim('Emmanuel')) and trim(time_slot) = '1.30 to 4';
update public.portal_roster_rows set area = 'Hub Room', updated_at = now() where status = 'active' and session_date = '2026-06-15'::date and lower(trim(client_name)) = lower(trim('Emmanuel')) and trim(time_slot) = '11 to 12.30';
update public.portal_roster_rows set area = 'Big Pool', updated_at = now() where status = 'active' and session_date = '2026-06-15'::date and lower(trim(client_name)) = lower(trim('Emmanuel')) and trim(time_slot) = '12.30 to 1.30';
