-- Shared achievement pool: co-instructors attach any staff draft for participant+day (up to 10).
...
comment on function public.portal_finalize_achievement_photos(uuid, uuid[], text, date) is
  'Attach up to 10 draft achievement photos/videos (any staff) to session feedback; archive remaining drafts for that participant+day.';
