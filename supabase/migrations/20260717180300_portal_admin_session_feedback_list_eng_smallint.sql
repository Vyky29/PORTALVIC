-- Fix RPC return type: session_feedback.engagement_rating is smallint, not numeric.
-- Mismatch caused: "structure of query does not match function result type"
-- which broke admin Sessions / participant feedback (RPC empty → stale workbook only).
...
comment on function public.portal_admin_session_feedback_list(date) is
  'Admin Sessions hub: session_feedback rows for exec/admin staff (engagement_rating smallint matches table).';
