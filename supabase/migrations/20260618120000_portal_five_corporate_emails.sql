...
-- Placeholders: solo borrar si no hay filas hijas (evita romper session_feedback / DM).
delete from auth.users u
where lower(u.email) = lower('stf013@staff.import.pending')
  and exists (select 1 from auth.users c where lower(c.email) = lower('victor@clubsensational.org'))
  and not exists (select 1 from public.session_feedback sf where sf.submitted_by_user_id = u.id)
  and not exists (select 1 from public.portal_staff_dm_messages m where m.author_id = u.id);
-- Repetir stf017/018/019 solo tras comprobar FKs, o desactivar placeholders en Auth manualmente.

-- Perfiles staff huérfanos: no borrar Auth con datos (session_feedback, DM, etc.).
-- Revisar manualmente en SQL si hace falta; usar _portal_relink_auth_user_fks(old, new).
