-- Purge Emanuel / Timi roster + feedback before their real start dates.
-- Emanuel: from 2026-06-12 (Mon/Wed/Fri day centre). Timi: from 2026-06-15 (tentative).
-- Keeps May Timi history; drops false June term rows and Supabase roster overrides.

begin;

create temporary table _portal_early_emanuel_timi_keys on commit drop as
select portal_session_key, session_date, client_name
from public.session_feedback
where (
  lower(trim(client_name)) = 'emanuel'
...
using _portal_early_emanuel_timi_keys k
...
where (
  lower(trim(pr.client_name)) = 'emanuel'
  and (
